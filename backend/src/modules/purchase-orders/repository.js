...
const findAll = async (tenantId, queryParams) => {
  ...
  const baseSql = `SELECT po.*, v.name AS vendor_name, w.name AS warehouse_name
  ...
  await run(trx,
    `INSERT INTO purchase_orders
       (id, tenant_id, po_number, vendor_id, warehouse_id, status, order_date, expected_date,
        total_amount, discount_amount, tax_amount, grand_total, notes, created_by)
     VALUES (?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      data.tenant_id,
...
const createPOItem = async (item, trx = null) => {
  const id = uuidv4();
  const lineTotal =
    (parseFloat(item.ordered_boxes) || 0) * (parseFloat(item.unit_price) || 0) *
...
const updatePO = async (id, tenantId, data, trx = null) => {
  const updates = [];
  const values = [];
  const allowed = ['vendor_id', 'warehouse_id', 'order_date', 'expected_date', 'notes', 'total_amount', 'discount_amount', 'tax_amount', 'grand_total'];
  ...
  await run(trx, `UPDATE purchase_orders SET ${updates.join(', ')} WHERE id = ? AND tenant_id = ?`, values);
};

const updatePOItem = async (itemId, purchaseOrderId, tenantId, data) => {
  const lineTotal =
    (parseFloat(data.ordered_boxes) || 0) * (parseFloat(data.unit_price) || 0) *
...
const deleteAllPOItems = async (purchaseOrderId, tenantId, trx = null) => {
  await run(trx, 'DELETE FROM purchase_order_items WHERE purchase_order_id = ? AND tenant_id = ?', [purchaseOrderId, tenantId]);
};

const setPOStatus = async (id, tenantId, status) => {
  await query('UPDATE purchase_orders SET status = ? WHERE id = ? AND tenant_id = ?', [status, id, tenantId]);
};
...
const findProductInPO = async (purchaseOrderId, productId, shadeId, excludeItemId, tenantId) => {
  ...
  let sql = `SELECT id FROM purchase_order_items WHERE purchase_order_id = ? AND product_id = ? AND tenant_id = ?`;
  if (shadeId) {
    sql += ` AND (shade_id <=> ?)`;
    params.push(shadeId);
  } else {
