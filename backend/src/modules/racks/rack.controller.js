...
const getRacks = async (req, res, next) => {
  try {
    const { data, meta } = await rackService.getAllRacks(req.tenantId, req.query);
...
const getRackById = async (req, res, next) => {
  try {
    const rack = await rackService.getRackById(req.params.id, req.tenantId);
...
const updateRack = async (req, res, next) => {
  try {
    ...
    const updated = await rackService.updateRack(req.params.id, req.tenantId, value);
    if (!updated) throw new AppError('Rack not found', 404, 'NOT_FOUND');
...
const deleteRack = async (req, res, next) => {
  try {
    const deleted = await rackService.deleteRack(req.params.id, req.tenantId);
    if (!deleted) throw new AppError('Rack not found', 404, 'NOT_FOUND');
