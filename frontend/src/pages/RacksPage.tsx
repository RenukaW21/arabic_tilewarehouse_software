...
export default function RacksPage() {
  ...
  const warehouseOptions =
    warehousesData?.data?.map((w) => ({ value: w.id, label: `${w.code} - ${w.name}` })) ?? [];
  ...
  const columns = [
    ...
    { key: "is_active", label: "Status", render: (r: Rack) => <StatusBadge status={r.is_active ? "active" : "inactive"} /> },
    {
      key: "actions",
      label: "Actions",
      render: (r: Rack) => (
  ...
  const formFields: FieldDef[] = fields.map((f) =>
    f.key === "warehouse_id" ? { ...f, options: warehouseOptions.length ? warehouseOptions : [{ value: "", label: "Loading warehouses..." }] } : f
  );
  ...
  return (
    <div>
      ...
      <DataTableShell<Rack>
        ...
        columns={columns}
        searchKey="name"
        searchPlaceholder="Search by name or aisle..."
        serverSide
